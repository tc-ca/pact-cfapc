...
Transport Canada requires the following points of contact to register an air carrier with PACT and begin the onboarding process. See [Registration](#registration) for more details.
...
The PACT system uses a web Application Programming Interface (API) to send and receive messages from participants in either Cargo-XML or C-IMP/CAMIR formats. More information can be found in section [Message Formats](#message-formats) and [Supported Versions for PACT Messages](#supported-versions-for-pact-messages).
...
To begin the registration process, contact Transport Canada at <pact-information-cfapc@tc.gc.ca> and identify the contacts listed in [Mandatory Points of Contacts](#mandatory-points-of-contact). A PACT representative will reply within 2 business days to obtain additional information and complete the registration.
...
These synchronous acknowledgement and error messages are distinct from the asynchronous two-way messages that can be enabled for “assessment complete” notifications and risk mitigation actions. See section 2.2.3 Two-Way Messaging.
...
- Preferred message format (Cargo-XML or C-IMP) and version (See [Supported Versions for PACT Messages](#supported-versions-for-pact-messages)).
...
| Headers    | A list of key value pairs added to requests/responses that provide additional information  |      “Content-Type”: “text/plain”; “Content-Type”; “application/xml”; “X-MYAIRLINE-REQUIRED-HEADER”: “Custom provided header”| 
|              |         |            | 
...
The PACT system can also accept additional data elements as long as the mandatory elements are present.
...
|**Responsible-Party** (if applicable)|  A seven-digit code that identifies the party responsible for receiving and responding to risk mitigation notifications, depending on the data submitter.  See Responsible PartyResponsible Party for more details               |
...
The test environment allows participants to test their data submissions and to receive responses from PACT in a simulated space that will not affect their actual operations. This also provides PACT an opportunity to familiarize itself with the air carrier’s data and identify any compliance issues well in advance of the effective date.

The production environment provides the greatest stability and performance and will be made available to participants once they have successfully completed testing in the test environment.
...
When a satisfactory amount of message types, volume, and errors have been tested, participants will then be invited to submit all Canada-bound live data to the test environment.

#### <a name="testing-data-submission"></a>Testing Data Submission

Testing in the test environment should start with 1-2 batches of 20-100 data submissions using as much “real-world” data as possible. These will be vetted by a PACT representative and the participant will be notified by PACT if there were any issues that need to be resolved or if additional test data is required. These batches will be used to test the PACT participant’s ability to transmit waybill and manifest data to PACT and receive synchronous acknowledgement messages or errors in response.
...
| Cargo-XML Message type   | Description |
| :---        |:----        |
...
|INVALID_HOUSE_BILL_NBR | House waybill number is not correct| **PER** 007-12345678-HAW0123456 ERR/INVALID_HOUSE_BILL_NBR |
...
| INVALID_BILL_QTY | Waybill piece count is not a number or could be extracted |**PER** 007-12345678-HAW0123456 ERR/INVALID_BILL_QTY |
...
<img src="images/media/image6.png" style="width:2.25in;height:1.78125in" alt="A screenshot of a computer code showing an error response from PACT in C-IMP format." />
...
##### Cargo-XML Master Air Waybill Submission Example
...
##### Cargo-XML Response (Acknowledgement-No Errors)
...
##### C-XML Flight Manifest (FFM) Example
...
##### C-IMP Master Waybill Submission Example
...
##### C-IMP Acknowledgement Response Example
...
##### C-IMP House Waybill Submission with Errors Example
...
##### C-IMP Flight Manifest (FFM) Example
...
If any participants or data submitters currently submit data to the PACT prototype, Transport Canada is grateful to those who provided essential feedback and helped contribute to the redesign of PACT. Transport Canada will contact those participants and data submitters to provide separate upgrade instructions for migrating to the new PACT version.
