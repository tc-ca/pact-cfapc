...
[2.1 PACT Targeting Process Flow [12](#pact-targeting-process-flow)](#pact-targeting-process-flow)
...
[3.1 Message Formats [29](#message-formats)](#message-formats)
...
[XCSN Example—CSN Response (RFI) [40](#_Toc194063876)](#_Toc194063876)
...
[Master Waybill Submission Example [54](#master-waybill-submission-example)](#master-waybill-submission-example)
...
<td>June 12, 2026</td>
<td>1.10</td>
<td>1.4.2 Eligible Participants (Other Data Submitters)</td>
<td>Added context on roles of different participants</td>
</tr>
<tr>
<td></td>
<td></td>
<td>2.2.5.1 PACT Data Elements</td>
<td>Added examples of cargo descriptions</td>
</tr>
<tr>
<td></td>
<td></td>
<td>2.3.2.1 B. PACT Support During Outages</td>
<td>Added a requirement to include specific information when notifying of a planned outage.</td>
</tr>
</tbody>
</table>
...
As of April 1, 2025, air carriers transporting cargo to Canada from international destinations are required to submit data to PACT and, in some cases, apply risk mitigation measures to specific shipments as per the *Canadian Aviation Security Regulations, 2012*.
...
This guide is intended for the implementation team or technician who is configuring a data submitter’s connection to the PACT system.
...
Other supply chain partners, such as freight forwarders or air carriers who create waybills or operate flights that precede the last point of departure for Canada (e.g., codeshare arrangements) may also participate in PACT voluntarily **<u>at the request of an air carrier</u>**.

Air carriers must identify their service providers and supply chain partners upon registration to authorize the partner’s connection to the PACT system. Air carriers must also identify the role(s) of their partners (e.g. data submission of some/all data elements (7+1 and flight data) and response capabilities (addressing errors, risk mitigations). These roles will determine which parts of this guide will be applicable to those partners. Roles and responsibilities will be discussed with a PACT representative during the onboarding process.

**Note**: While air carriers may delegate certain requirements, they are ultimately liable for compliance with the *Canadian Aviation Security Regulations, 2012*.
...
The amendments to the *Canadian Aviation Security Regulations, 2012 (Air Cargo)* came into force on April 1, 2025. Air carriers are expected to submit the required data elements to the PACT system and to respond to Transport Canada requests (RFIs, RFSs, DNLs) or they can be subject to administrative monetary penalties (AMPs).
...
Participants should familiarize themselves with what notifications they can expect via the PACT system versus email based on their role (i.e. mandatory vs eligible participant). In the case where a risk mitigation action is required, the PACT system is capable of two-way messaging to ensure RFIs, RFSs and DNLs are communicated in near-real time. See section [2.2.3 Two-Way Messaging](#two-way-messaging) for more details.

## PACT Onboarding

Before submitting data to PACT, participants must contact Transport Canada for a one-time registration and setup, which includes identifying contacts and providing authorizations for any third-party that will be submitting data to PACT on behalf of or in addition to the air carrier, if applicable.
...
**Note**: It is recommended that air carriers complete this process several months before they’re scheduled to operate cargo-carrying flights to Canada.
...
**Note:** Consolidation messages do not typically satisfy these requirements as they may lack the original shipper names and addresses and a piece-level description of the cargo. While the system will accept “consol”, “consolidation”, “courier bag”, or “clvs” for the cargo description, this would not be considered compliant with the regulations, in which case all associated house waybills would be required.
...
<td><p>The three-digit IATA code for the air carrier that is transporting the cargo to Canada</p>
...
<td><p>A seven-digit code that identifies the party responsible for receiving and responding to risk mitigation notifications, if different from the air carrier.</p>
...
The Responsible Party header value is a 7-digit code that is used to identify a third party that is responsible for responding to all risk mitigation notifications initiated by PACT and that is not the air carrier. The responsible party identifier should only be applied as directed by the PACT team.
...
- Waybill with a commodity description from Unacceptable Commodity Descriptions to test error message
...
After a successful test of risk mitigation procedures, the participant will be approved to move to the PACT production system. During this period, PACT representatives will work with participants to resolve any technical issues that may arise in the transition to the production environment. **Note**: The PACT test environment does not have 24-hour support.

###  PACT Production Environment

After sufficient testing in the PACT test environment and approval from a PACT representative from Transport Canada, participants granted access to the PACT production environment. All air carriers must be fully operational in the production environment in advance of their effective date of operations to Canada.
...
Air carriers with a planned period of system unavailability must notify Transport Canada at <PACT-Information-CFAPC@tc.gc.ca> at least three (3) business days before the planned downtime and include the following information:

- Date and estimated duration of the downtime; and

- Affected functions (e.g. will the required data elements still be submitted during the outage? Will two-way messaging (if enabled) still function (i.e. will the system receive “assessment complete” and RFI/RFS/DNL messages)?)

The PACT team will then provide instructions on a case-by-case basis in those circumstances.
